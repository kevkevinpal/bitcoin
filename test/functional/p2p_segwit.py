...
class TestP2PConn(P2PInterface):
    ...
    def announce_block_and_wait_for_getdata(self, block, use_header, timeout=60):
        ...
        if use_header:
            ...
            message_inv = msg_inv(inv=[CInv(MSG_BLOCK, block.sha256)])
            self.send_message(message_inv)
            headers = self.last_message['getheaders']
            self.wait_for_getheaders(headers.locator.vHave[0])
            self.send_message(msg)
