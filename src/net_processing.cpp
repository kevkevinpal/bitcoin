...
struct Peer {
    ...
    {
        return log_ip ? strprintf(" peeraddr=%s", m_addr.ToStringAddrPort()) : "";
    }
...
private:
    ...
    void EvictExtraOutboundPeers(std::chrono::seconds now) EXCLUSIVE_LOCKS_REQUIRED(cs_main, !m_peer_mutex);
    ...
    void RequestDisconnect(Peer& peer);
    /**
     ...
     */
    bool MaybeDiscourageAndDisconnect(Peer& peer);
    ...
    */
    void ProcessHeadersMessage(Peer& peer,
    ...
    void HandleUnconnectingHeaders(Peer& peer, const std::vector<CBlockHeader>& headers) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
     ...
     */
    bool IsContinuationOfLowWorkHeadersSync(Peer& peer,
            std::vector<CBlockHeader>& headers)
     ...
     */
    bool TryLowWorkHeadersSync(Peer& peer,
     ...
     */
    bool MaybeSendGetHeaders(const CBlockLocator& locator, Peer& peer) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
    /** Potentially fetch blocks from this peer upon receipt of a new headers tip */
    void HeadersDirectFetchBlocks(const Peer& peer, const CBlockIndex& last_header);
    /** Update peer state based on received headers message */
    void UpdatePeerStateForReceivedHeaders(Peer& peer, const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
     ...
     *  to time out. */
    void MaybeSendPing(Peer& peer, std::chrono::microseconds now);

    /** Send `addr` messages on a regular schedule. */
    void MaybeSendAddr(Peer& peer, std::chrono::microseconds current_time) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);

    /** Send a single `sendheaders` message, after we have completed headers sync with a peer. */
    void MaybeSendSendHeaders(Peer& peer) EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex);
    ...
    bool RejectIncomingTxs(const Peer& peer) const;
    ...
    */
    ...
    void ProcessBlock(Peer& peer, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked);

    /** Process compact block txns  */
    void ProcessCompactBlockTxns(Peer& peer, const BlockTransactions& block_transactions)
    ...
    void ProcessGetBlockData(Peer& peer, const CInv& inv)
        EXCLUSIVE_LOCKS_REQUIRED(g_msgproc_mutex, !m_most_recent_block_mutex);
     ...
     */
    bool PrepareBlockFilterRequest(Peer& peer,
    ...
    void LogBlockHeader(const CBlockIndex& index, const Peer& peer, bool via_compact_block);
...
void PeerManagerImpl::PushNodeVersion(const Peer& peer)
{
    uint64_t my_services{peer.m_our_services};
    const int64_t nTime{count_seconds(GetTime<std::chrono::seconds>())};
    uint64_t nonce = peer.GetLocalNonce();
    const int nNodeStartingHeight{m_best_height};
    NodeId nodeid = peer.m_id;
    CAddress addr = peer.m_addr;

    CService addr_you = addr.IsRoutable() && !IsProxy(addr) && addr.IsAddrV1Compatible() ? addr : CService();
    uint64_t your_services{addr.nServices};

    const bool tx_relay{!RejectIncomingTxs(peer)};
...
void PeerManagerImpl::FinalizeNode(NodeId nodeid)
{
...
std::optional<std::string> PeerManagerImpl::FetchBlock(NodeId peer_id, const CBlockIndex& block_index)
{
    ...
    if (peer->m_disconnecting || !peer->m_handshake_complete) return "Peer not fully connected";
    MakeAndPushMessage(peer_id, NetMsgType::GETDATA, invs);
...
void PeerManagerImpl::ProcessGetBlockData(Peer& peer, const CInv& inv)
{
        ...
        if (inv.IsMsgBlk()) {
            ...
            if (sendMerkleBlock) {
                ...
                for (const auto& [tx_idx, _] : merkleBlock.vMatchedTxn)
                    MakeAndPushMessage(peer.m_id, NetMsgType::TX, TX_NO_WITNESS(*pblock->vtx[tx_idx]));
            }
            // else
            // no response
...
void PeerManagerImpl::SendBlockTransactions(Peer& peer, const CBlock& block, const BlockTransactionsRequest& req)
{
    ...
    LogDebug(BCLog::CMPCTBLOCK, "Peer %d sent us a GETBLOCKTXN for block %s, sending a BLOCKTXN with %u txns. (%u bytes)\n", peer.m_id, block.GetHash().ToString(), resp.txn.size(), tx_requested_size);
 ...
 */
void PeerManagerImpl::HandleUnconnectingHeaders(Peer& peer,
        const std::vector<CBlockHeader>& headers)
{
    auto node_id = peer.m_id;
    // Try to fill in the missing headers.
    const CBlockIndex* best_header{WITH_LOCK(cs_main, return m_chainman.m_best_header)};
    if (MaybeSendGetHeaders(GetLocator(best_header), peer)) {
    ...
    WITH_LOCK(cs_main, UpdateBlockAvailability(node_id, headers.back().GetHash()));
}
...
bool PeerManagerImpl::IsContinuationOfLowWorkHeadersSync(Peer& peer, std::vector<CBlockHeader>& headers)
{
    auto node_id = peer.m_id;
    if (peer.m_headers_sync) {
        ...
        if (result.request_more) {
            ...
            if (!locator.vHave.empty()) {
                ...
                bool sent_getheaders = MaybeSendGetHeaders(locator, peer);
                Assume(sent_getheaders);
                LogDebug(BCLog::NET, "more getheaders (from %s) to peer=%d\n",
                    locator.vHave.front().ToString(), node_id);
            }
        }
        ...
        if (peer.m_headers_sync->GetState() == HeadersSyncState::State::FINAL) {
            ...
            LOCK(m_headers_presync_mutex);
            m_headers_presync_stats.erase(node_id);
        } else {
            ...
            LOCK(m_headers_presync_mutex);
            m_headers_presync_stats[node_id] = stats;
            ...
            if (best_it == m_headers_presync_stats.end()) {
                ...
                best_updated = (peer_best == node_id);
            } else if (best_it->first == node_id || stats > best_it->second) {
                // pfrom was and remains the best peer, or pfrom just became best.
                m_headers_presync_bestpeer = node_id;
                best_updated = true;
            }
...
bool PeerManagerImpl::TryLowWorkHeadersSync(Peer& peer, const CBlockIndex* chain_start_header, std::vector<CBlockHeader>& headers)
{
    ...
    if (total_work < minimum_chain_work) {
        ...
        if (headers.size() == m_opts.max_headers_result) {
            ...
            (void)IsContinuationOfLowWorkHeadersSync(peer, headers);
        } else {
            LogDebug(BCLog::NET, "Ignoring low-work chain (height=%u) from peer=%d\n", chain_start_header->nHeight + headers.size(), peer.m_id);
        }
...
bool PeerManagerImpl::MaybeSendGetHeaders(const CBlockLocator& locator, Peer& peer)
{
 ...
 */
void PeerManagerImpl::HeadersDirectFetchBlocks(const Peer& peer, const CBlockIndex& last_header)
{
    auto node_id = peer.m_id;
    LOCK(cs_main);
    ...
    if (CanDirectFetch() && last_header.IsValid(BLOCK_VALID_TREE) && m_chainman.ActiveChain().Tip()->nChainWork <= last_header.nChainWork) {
        ...
        if (!m_chainman.ActiveChain().Contains(pindexWalk)) {
            ...
            for (const CBlockIndex* pindex : vToFetch | std::views::reverse) {
                ...
                BlockRequested(node_id, *pindex);
                LogDebug(BCLog::NET, "Requesting block %s from  peer=%d\n",
                        pindex->GetBlockHash().ToString(), node_id);
            }
            if (vGetData.size() > 1) {
 ...
 */
void PeerManagerImpl::UpdatePeerStateForReceivedHeaders(Peer& peer,
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    UpdateBlockAvailability(node_id, last_header.GetBlockHash());
    ...
    if (m_chainman.IsInitialBlockDownload() && !may_have_more_headers) {
        ...
        if (nodestate->pindexBestKnownBlock && nodestate->pindexBestKnownBlock->nChainWork < m_chainman.MinimumChainWork()) {
            ...
            if (IsOutboundOrBlockRelayConn(peer.m_conn_type)) {
    ...
    if (!peer.m_disconnecting && IsFullOutboundConn(peer.m_conn_type) && nodestate->pindexBestKnownBlock != nullptr) {
        if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && nodestate->pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
            LogDebug(BCLog::NET, "Protecting outbound peer=%d from eviction\n", node_id);
            nodestate->m_chain_sync.m_protect = true;
            m_evictionman.UpdateProtected(node_id);
            ++m_outbound_peers_with_protect_from_disconnect;
        }
    }
}

void PeerManagerImpl::ProcessHeadersMessage(Peer& peer,
                                            std::vector<CBlockHeader>&& headers,
                                            bool via_compact_block)
{
    auto node_id = peer.m_id;
    ...
    bool have_headers_sync = false;
    {
        ...
        already_validated_work = IsContinuationOfLowWorkHeadersSync(peer, headers);
    ...
    if (!headers_connect_blockindex) {
        ...
        // could be benign.
        HandleUnconnectingHeaders(peer, headers);
        return;
    }
    ...
    // processing.
    if (!already_validated_work && TryLowWorkHeadersSync(peer,
    ...
    if (!processed) {
        if (state.IsInvalid()) {
            MaybePunishNodeForBlock(node_id, state, via_compact_block, "invalid header received");
            return;
        }
    }
    assert(pindexLast);
    ...
    if (processed && received_new_header) {
        LogBlockHeader(*pindexLast, peer, /*via_compact_block=*/false);
    }

    // Consider fetching more headers if we are not using our headers-sync mechanism.
    if (nCount == m_opts.max_headers_result && !have_headers_sync) {
        // Headers message had its maximum size; the peer may have more headers.
        if (MaybeSendGetHeaders(GetLocator(pindexLast), peer)) {
            LogDebug(BCLog::NET, "more getheaders (%d) to end to peer=%d (startheight:%d)\n",
                    pindexLast->nHeight, node_id, peer.m_starting_height);
        }
    }

    UpdatePeerStateForReceivedHeaders(peer, *pindexLast, received_new_header, nCount == m_opts.max_headers_result);

    // Consider immediately downloading blocks.
    HeadersDirectFetchBlocks(peer, *pindexLast);
...
bool PeerManagerImpl::PrepareBlockFilterRequest(Peer& peer,
                                                BlockFilterType filter_type, uint32_t start_height,
                                                const uint256& stop_hash, uint32_t max_height_diff,
                                                const CBlockIndex*& stop_index,
                                                BlockFilterIndex*& filter_index)
{
    ...
    if (!supported_filter_type) {
        LogDebug(BCLog::NET, "peer requested unsupported block filter type: %d, %s\n",
                 static_cast<uint8_t>(filter_type), peer.DisconnectMsg(fLogIPs));
        RequestDisconnect(peer);
        return false;
    }

    {
        ...
        if (!stop_index || !BlockRequestAllowed(stop_index)) {
            LogDebug(BCLog::NET, "peer requested invalid block hash: %s, %s\n",
                     stop_hash.ToString(), peer.DisconnectMsg(fLogIPs));
            RequestDisconnect(peer);
            return false;
        }
    }
    ...
    if (start_height > stop_height) {
        LogDebug(BCLog::NET, "peer sent invalid getcfilters/getcfheaders with "
                 ...
                 start_height, stop_height, peer.DisconnectMsg(fLogIPs));
        RequestDisconnect(peer);
        return false;
    }
    if (stop_height - start_height >= max_height_diff) {
        LogDebug(BCLog::NET, "peer requested too many cfilters/cfheaders: %d / %d, %s\n",
                 stop_height - start_height + 1, max_height_diff, peer.DisconnectMsg(fLogIPs));
        RequestDisconnect(peer);
        return false;
    }
...
void PeerManagerImpl::ProcessBlock(Peer& peer, const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked)
{
...
void PeerManagerImpl::ProcessCompactBlockTxns(Peer& peer, const BlockTransactions& block_transactions)
{
    auto node_id = peer.m_id;
    ...
    bool fBlockRead{false};
    {
        ...
        bool first_in_flight = already_in_flight == 0 || (range_flight.first->second.first == node_id);

        while (range_flight.first != range_flight.second) {
            auto [found_id, block_it] = range_flight.first->second;
            if (found_id == node_id && block_it->partialBlock) {
        ...
        if (partialBlock.header.IsNull()) {
            ...
            RemoveBlockRequest(block_transactions.blockhash, node_id);
            Misbehaving(peer, "previous compact block reconstruction attempt failed");
            LogDebug(BCLog::NET, "Peer %d sent compact block transactions multiple times", node_id);
            return;
        }
        ...
        if (status == READ_STATUS_INVALID) {
            RemoveBlockRequest(block_transactions.blockhash, node_id); // Reset in-flight state in case Misbehaving does not result in a disconnect
            ...
            RemoveBlockRequest(block_transactions.blockhash, node_id); // it is now an empty pointer
            fBlockRead = true;
            ...
            // out to be invalid.
            mapBlockSource.emplace(block_transactions.blockhash, std::make_pair(node_id, false));
        }
    ...
    if (fBlockRead) {
        ...
        ProcessBlock(peer, pblock, /*force_processing=*/true, /*min_pow_checked=*/true);
    }
    return;
}

void PeerManagerImpl::LogBlockHeader(const CBlockIndex& index, const Peer& peer, bool via_compact_block) {
                                     ...
                                     const std::chrono::microseconds time_received)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (!IsBlockOnlyConn(peer->m_conn_type) &&
            !IsFeelerConn(peer->m_conn_type) &&
            ...
            if (fRelay) {
                m_evictionman.UpdateRelayTxs(node_id);
            }
        }
        ...
        if (greatest_common_version >= WTXID_RELAY_VERSION && m_txreconciliation) {
            ...
            if (tx_relay && WITH_LOCK(tx_relay->m_bloom_filter_mutex, return tx_relay->m_relay_txs) &&
                ...
                const uint64_t recon_salt = m_txreconciliation->PreRegisterPeer(node_id);
        ...
        // Potentially mark this peer as a preferred download peer.
        {
            LOCK(cs_main);
            CNodeState* state = State(node_id);
            state->fPreferredDownload = (!IsInboundConn(peer->m_conn_type) || peer->HasPermission(NetPermissionFlags::NoBan)) && !IsAddrFetchConn(peer->m_conn_type) && CanServeBlocks(*peer);
            m_num_preferred_download_peers += state->fPreferredDownload;
        }
        ...
        if (IsFeelerConn(peer->m_conn_type)) {
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (m_txreconciliation) {
            if (!peer->m_wtxid_relay || !m_txreconciliation->IsPeerRegistered(node_id)) {
                ...
                m_txreconciliation->ForgetPeer(node_id);
            }
        }
        ...
        {
            LOCK2(::cs_main, m_tx_download_mutex);
            const CNodeState* state = State(node_id);
            m_txdownloadman.ConnectedPeer(node_id, node::TxDownloadConnectionInfo {
                .m_preferred = state->fPreferredDownload,
                .m_relay_permissions = peer->HasPermission(NetPermissionFlags::Relay),
    ...
    if (msg_type == NetMsgType::SENDTXRCNCL) {
        ...
        const ReconciliationRegisterResult result = m_txreconciliation->RegisterPeer(node_id, IsInboundConn(peer->m_conn_type),
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        const bool rate_limited = !peer->HasPermission(NetPermissionFlags::Addr);
        uint64_t num_proc = 0;
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            if (addr.nTime > current_a_time - 10min && !peer->m_getaddr_sent && vAddr.size() <= 10 && addr.IsRoutable()) {
                // Relay to a limited number of other nodes
                RelayAddress(node_id, addr, reachable);
            }
        ...
        LogDebug(BCLog::NET, "Received addr: %u addresses (%u processed, %u rate-limited) from peer=%d\n",
                 vAddr.size(), num_proc, num_rate_limit, node_id);

        m_addrman.Add(vAddrOk, peer->m_addr, 2h);
        if (vAddr.size() < 1000) peer->m_getaddr_sent = false;

        // AddrFetch: Require multiple addresses to avoid disconnecting on self-announcements
        if (IsAddrFetchConn(peer->m_conn_type) && vAddr.size() > 1) {
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        const bool reject_tx_invs{RejectIncomingTxs(*peer)};
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                LogDebug(BCLog::NET, "got inv: %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", node_id);

                UpdateBlockAvailability(node_id, inv.hash);
                ...
                if (!m_chainman.IsInitialBlockDownload()) {
                    const bool fAlreadyHave{m_txdownloadman.AddTxAnnouncement(node_id, gtxid, current_time)};
                    LogDebug(BCLog::NET, "got inv: %s  %s peer=%d\n", inv.ToString(), fAlreadyHave ? "have" : "new", node_id);
                }
            } else {
                LogDebug(BCLog::NET, "Unknown inv type \"%s\" received from peer=%d\n", inv.ToString(), node_id);
            }
        }
        ...
        if (best_block != nullptr) {
            ...
            if (state.fSyncStarted || (!peer->m_inv_triggered_getheaders_before_sync && *best_block != m_last_block_inv_triggering_headers_sync)) {
                if (MaybeSendGetHeaders(GetLocator(m_chainman.m_best_header), *peer)) {
    ...
    if (msg_type == NetMsgType::GETBLOCKS) {
        ...
        if (locator.vHave.size() > MAX_LOCATOR_SZ) {
            LogDebug(BCLog::NET, "getblocks locator size %lld > %d, %s\n", locator.vHave.size(), MAX_LOCATOR_SZ, peer->DisconnectMsg(fLogIPs));
            RequestDisconnect(*peer);
            return;
        }
        ...
        int nLimit = 500;
        LogDebug(BCLog::NET, "getblocks %d to %s limit %d from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), nLimit, node_id);
    ...
    if (msg_type == NetMsgType::GETBLOCKTXN) {
        ...
        if (recent_block) {
            SendBlockTransactions(*peer, *recent_block, req);
            return;
        }
        ...
        if (!block_pos.IsNull()) {
            ...
            SendBlockTransactions(*peer, block, req);
            return;
        }
    ...
    if (msg_type == NetMsgType::GETHEADERS) {
        ...
        if (m_chainman.ActiveTip() == nullptr ||
                (m_chainman.ActiveTip()->nChainWork < m_chainman.MinimumChainWork() && !peer->HasPermission(NetPermissionFlags::Download))) {
        ...
        LogDebug(BCLog::NET, "getheaders %d to %s from peer=%d\n", (pindex ? pindex->nHeight : -1), hashStop.IsNull() ? "end" : hashStop.ToString(), node_id);
        for (; pindex; pindex = m_chainman.ActiveChain().Next(pindex))
        {
    ...
    if (msg_type == NetMsgType::TX) {
        if (RejectIncomingTxs(*peer)) {
            LogDebug(BCLog::NET, "transaction sent in violation of protocol, %s", peer->DisconnectMsg(fLogIPs));
            RequestDisconnect(*peer);
            return;
        }
        ...
        const auto& [should_validate, package_to_validate] = m_txdownloadman.ReceivedTx(node_id, ptx);
        if (!should_validate) {
            if (peer->HasPermission(NetPermissionFlags::ForceRelay)) {
                // Always relay transactions received from peers with forcerelay
                // permission, even if they were already in the mempool, allowing
                // the node to function as a gateway for nodes hidden behind it.
                if (!m_mempool.exists(txid)) {
                    LogPrintf("Not relaying non-mempool transaction %s (wtxid=%s) from forcerelay peer=%d\n",
                              txid.ToString(), wtxid.ToString(), node_id);
                } else {
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (!prev_block) {
            ...
            if (!m_chainman.IsInitialBlockDownload()) {
                MaybeSendGetHeaders(GetLocator(m_chainman.m_best_header), *peer);
            }
            return;
        ...
        if (!m_chainman.ProcessNewBlockHeaders({{cmpctblock.header}}, /*min_pow_checked=*/true, state, &pindex)) {
            if (state.IsInvalid()) {
                MaybePunishNodeForBlock(node_id, state, /*via_compact_block=*/true, "invalid header via cmpctblock");
                return;
            }
        }
        ...
        if (received_new_header) {
            LogBlockHeader(*pindex, *peer, /*via_compact_block=*/true);
        }
        ...
        {
        LOCK(cs_main);
        UpdateBlockAvailability(node_id, pindex->GetBlockHash());
        ...
        while (range_flight.first != range_flight.second) {
            if (range_flight.first->second.first == node_id) {
                requested_block_from_this_peer = true;
                break;
            }
            range_flight.first++;
        ...
        if (pindex->nHeight <= m_chainman.ActiveChain().Height() + 2) {
            if ((already_in_flight < MAX_CMPCTBLOCKS_INFLIGHT_PER_BLOCK && nodestate->vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) ||
                ...
                if (!BlockRequested(node_id, *pindex, &queuedBlockIt)) {
                ...
                if (status == READ_STATUS_INVALID) {
                    RemoveBlockRequest(pindex->GetBlockHash(), node_id); // Reset in-flight state in case Misbehaving does not result in a disconnect
                    ...
                    if (first_in_flight)  {
                        ...
                        RemoveBlockRequest(pindex->GetBlockHash(), node_id);
                    }
                    return;
                }
        ...
        if (fProcessBLOCKTXN) {
            ...
            return ProcessCompactBlockTxns(*peer, txn);
        }
        ...
        if (fRevertToHeaderProcessing) {
            ...
            return ProcessHeadersMessage(*peer, {cmpctblock.header}, /*via_compact_block=*/true);
        }
        ...
        if (fBlockReconstructed) {
            ...
            {
                LOCK(cs_main);
                mapBlockSource.emplace(pblock->GetHash(), std::make_pair(node_id, false));
            }
    ...
    if (msg_type == NetMsgType::BLOCKTXN)
    {
        ...
        return ProcessCompactBlockTxns(*peer, resp);
    }
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        ProcessHeadersMessage(*peer, std::move(headers), /*via_compact_block=*/false);
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
        ...
        bool min_pow_checked = false;
        {
            ...
            mapBlockSource.emplace(hash, std::make_pair(node_id, true));
        ...
        ProcessBlock(*peer, pblock, forceProcessing, min_pow_checked);
        return;
    }
    ...
    if (msg_type == NetMsgType::GETADDR) {
        ...
        if (peer->HasPermission(NetPermissionFlags::Addr)) {
            vAddr = m_connman->getAddressesUnsafe(MAX_ADDR_TO_SEND, MAX_PCT_ADDR_TO_SEND);
        } else {
    ...
    if (msg_type == NetMsgType::MEMPOOL) {
        ...
        if (!(peer->m_our_services & NODE_BLOOM) && !peer->HasPermission(NetPermissionFlags::Mempool))
        {
            if (!peer->HasPermission(NetPermissionFlags::NoBan))
            {
    ...
    if (msg_type == NetMsgType::FILTERLOAD) {
        ...
        if (!filter.IsWithinSizeConstraints())
        {
            ...
            m_evictionman.UpdateLoadedBloomFilter(node_id, true);
            m_evictionman.UpdateRelayTxs(node_id);
        }
        return;
    }
    ...
    if (msg_type == NetMsgType::FILTERCLEAR) {
        ...
        m_evictionman.UpdateLoadedBloomFilter(node_id, false);
        m_evictionman.UpdateRelayTxs(node_id);
        return;
    }
    ...
    if (msg_type == NetMsgType::FEEFILTER) {
        ...
        if (MoneyRange(newFeeFilter)) {
            ...
            LogDebug(BCLog::NET, "received: feefilter of %s from peer=%d\n", CFeeRate(newFeeFilter).ToString(), node_id);
        }
        return;
    }

    if (msg_type == NetMsgType::GETCFILTERS) {
        ProcessGetCFilters(*peer, vRecv);
        return;
    }

    if (msg_type == NetMsgType::GETCFHEADERS) {
        ProcessGetCFHeaders(*peer, vRecv);
        return;
    }

    if (msg_type == NetMsgType::GETCFCHECKPT) {
        ProcessGetCFCheckPt(*peer, vRecv);
        return;
    }
    ...
    if (msg_type == NetMsgType::NOTFOUND) {
        ...
        LOCK(m_tx_download_mutex);
        m_txdownloadman.ReceivedNotFound(node_id, tx_invs);
        return;
    }
...
bool PeerManagerImpl::MaybeDiscourageAndDisconnect(Peer& peer)
{
    {
        LOCK(peer.m_misbehavior_mutex);
    ...
    if (IsManualConn(peer.m_conn_type)) {
...
bool PeerManagerImpl::ProcessMessages(NodeId node_id)
{
    ...
    if (!IsInboundConn(peer->m_conn_type) && !peer->m_outbound_version_message_sent) return false;
...
void PeerManagerImpl::ConsiderEviction(Peer& peer, std::chrono::seconds time_in_seconds)
{
    AssertLockHeld(cs_main);
    auto node_id = peer.m_id;
    CNodeState &state = *State(node_id);

    if (!state.m_chain_sync.m_protect && IsOutboundOrBlockRelayConn(peer.m_conn_type) && state.fSyncStarted) {
        ...
        if (state.pindexBestKnownBlock != nullptr && state.pindexBestKnownBlock->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork) {
            ...
            if (state.m_chain_sync.m_sent_getheaders) {
                ...
                LogInfo("Outbound peer has old chain, best known block = %s, %s\n", state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", peer.DisconnectMsg(fLogIPs));
                RequestDisconnect(peer);
            } else {
                ...
                MaybeSendGetHeaders(
                        ...
                        peer);
                LogDebug(BCLog::NET, "sending getheaders to outbound peer=%d to verify chain work (current best known block:%s, benchmark blockhash: %s)\n", node_id, state.pindexBestKnownBlock != nullptr ? state.pindexBestKnownBlock->GetBlockHash().ToString() : "<none>", state.m_chain_sync.m_work_header->GetBlockHash().ToString());
...
void PeerManagerImpl::EvictExtraOutboundPeers(std::chrono::seconds now)
{
    ...
    if (GetExtraBlockRelayCount() > 0) {
        ...
        auto node_id = peer->m_id;
            ...
            CNodeState *node_state = State(node_id);
...
void PeerManagerImpl::MaybeSendPing(Peer& peer, std::chrono::microseconds now)
{
...
void PeerManagerImpl::MaybeSendAddr(Peer& peer, std::chrono::microseconds current_time)
{
...
void PeerManagerImpl::MaybeSendSendHeaders(Peer& peer)
{
...
void PeerManagerImpl::MaybeSendFeefilter(Peer& peer, std::chrono::microseconds current_time)
{
    if (m_opts.ignore_incoming_txs) return;
    if (peer.GetCommonVersion() < FEEFILTER_VERSION) return;
    // peers with the forcerelay permission should not filter txs to us
    if (peer.HasPermission(NetPermissionFlags::ForceRelay)) return;
...
bool PeerManagerImpl::RejectIncomingTxs(const Peer& peer) const
{
    // block-relay-only peers may never send txs to us
    if (IsBlockOnlyConn(peer.m_conn_type)) return true;
    if (IsFeelerConn(peer.m_conn_type)) return true;
...
bool PeerManagerImpl::SetupAddressRelay(Peer& peer)
{
    ...
    if (IsBlockOnlyConn(peer.m_conn_type)) return false;
...
bool PeerManagerImpl::SendMessages(NodeId peer_id)
{
    AssertLockNotHeld(m_tx_download_mutex);
    AssertLockHeld(g_msgproc_mutex);
    ...
    if (MaybeDiscourageAndDisconnect(*peer)) return true;

    // Initiate version handshake for outbound connections
    if (!IsInboundConn(peer->m_conn_type) && !peer->m_outbound_version_message_sent) {
        PushNodeVersion(*peer);
    ...
    MaybeSendPing(*peer, current_time);

    MaybeSendAddr(*peer, current_time);

    MaybeSendSendHeaders(*peer);

    {
        ...
        } else if (CanServeBlocks(*peer) && !IsAddrFetchConn(peer->m_conn_type)) {
        ...
        if (!state.fSyncStarted && CanServeBlocks(*peer) && !m_chainman.m_blockman.LoadingBlocks()) {
            ...
            if ((nSyncStarted == 0 && sync_blocks_and_headers_from_peer) || m_chainman.m_best_header->Time() > NodeClock::now() - 24h) {
                ...
                if (MaybeSendGetHeaders(GetLocator(pindexStart), *peer)) {
                    LogDebug(BCLog::NET, "initial getheaders (%d) to peer=%d (startheight:%d)\n", pindexStart->nHeight, peer_id, peer->m_starting_height);
        ...
        //
        {
            ...
            ProcessBlockAvailability(peer_id); // ensure pindexBestKnownBlock is up-to-date
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.m_requested_hb_cmpctblocks) {
                    ...
                    LogDebug(BCLog::NET, "%s sending header-and-ids %s to peer=%d\n", __func__,
                            vHeaders.front().GetHash().ToString(), peer_id);
                    ...
                    if (vHeaders.size() > 1) {
                        LogDebug(BCLog::NET, "%s: %u headers, range (%s, %s), to peer=%d\n", __func__,
                                ...
                                vHeaders.back().GetHash().ToString(), peer_id);
                    } else {
                ...
                if (!peer->m_blocks_for_headers_relay.empty()) {
                    ...
                    if (!PeerHasHeader(&state, pindex)) {
                        ...
                        LogDebug(BCLog::NET, "%s: sending inv peer=%d hash=%s\n", __func__,
                            peer_id, hashToAnnounce.ToString());
                    }
                }
            }
        ...
        if (auto tx_relay = peer->GetTxRelay(); tx_relay != nullptr) {
                ...
                bool fSendTrickle = peer->HasPermission(NetPermissionFlags::NoBan);
        ...
        if (state.vBlocksInFlight.size() > 0) {
            ...
            if (current_time > state.m_downloading_since + std::chrono::seconds{consensusParams.nPowTargetSpacing} * (BLOCK_DOWNLOAD_TIMEOUT_BASE + BLOCK_DOWNLOAD_TIMEOUT_PER_PEER * nOtherPeersWithValidatedDownloads)) {
                LogInfo("Timeout downloading block %s, %s\n", queuedBlock.pindex->GetBlockHash().ToString(), peer->DisconnectMsg(fLogIPs));
                RequestDisconnect(*peer);
                return true;
            }
        }
        ...
        ConsiderEviction(*peer, GetTime<std::chrono::seconds>());
        ...
        if (CanServeBlocks(*peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(*peer)) || !m_chainman.IsInitialBlockDownload()) && state.vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                uint32_t nFetchFlags = GetFetchFlags(*peer);
                vGetData.emplace_back(MSG_BLOCK | nFetchFlags, pindex->GetBlockHash());
                BlockRequested(peer_id, *pindex);
                LogDebug(BCLog::NET, "Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, peer_id);
            }
        ...
        {
            LOCK(m_tx_download_mutex);
            for (const GenTxid& gtxid : m_txdownloadman.GetRequestsToSend(peer_id, current_time)) {
    ...
    MaybeSendFeefilter(*peer, current_time);
    return true;
}
